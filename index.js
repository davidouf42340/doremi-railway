...
app.post('/webhook/orders-paid', async (req, res) => {
  ...
  // ── 3b. Sauvegarder le brief en Supabase ───────────────
  const recipientName = isDeuil ? brief['Prénom du proche'] : brief['Prénom destinataire'];
  const occasion = brief['Occasion'] || (isDeuil ? 'Hommage' : 'Festivité');
  ...
  try {
    await db.createOrder({
      ...
      customerName,
      type: isDeuil ? 'deuil' : 'festivites',
      briefData: brief,
      recipientName: recipientName || '',
      occasion,
    });
    console.log(`[Dorémi] Brief sauvegardé en Supabase pour commande #${order.order_number}`);
  } catch (e) {
    console.error('[Dorémi] Erreur Supabase (brief):', e.message);
  }
  ...
  // ── 5b. Sauvegarder les paroles en Supabase ────────────
  try {
    await db.saveLyrics(order.id, paroles);
    console.log(`[Dorémi] Paroles sauvegardées en Supabase pour commande #${order.order_number}`);
  } catch (e) {
    console.error('[Dorémi] Erreur Supabase (paroles):', e.message);
  }
...
app.post('/formulaire', async (req, res) => {
  ...
  // Récupérer les infos de commande Shopify
  ...
  // ── Sauvegarder le brief en metafield Shopify (backup immédiat) ──
  try {
    const briefJson = JSON.stringify(brief);
    const shop = (process.env.SHOPIFY_SHOP_DOMAIN || '').replace(/^https?:\/\//, '').trim();
    await fetch(`https://${shop}/admin/api/2024-01/orders/${order_id}/metafields.json`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
    ...
    console.log(`[Dorémi /formulaire] Brief sauvegardé en metafield Shopify pour commande ${order_id}`);
  } catch (e) {
    console.warn('[Dorémi /formulaire] Erreur sauvegarde metafield brief:', e.message);
  }

  // ── Sauvegarder le brief en Supabase ──
  const recipientName = isDeuil ? champs['Prénom du proche'] : champs['Prénom destinataire'];
  const occasion = champs['Occasion'] || (isDeuil ? 'Hommage' : 'Festivité');
  const customerName = order.billing_address
    ? `${order.billing_address.first_name || ''} ${order.billing_address.last_name || ''}`.trim()
    : '';
  ...
  let dbOrder;
  try {
    dbOrder = await db.createOrder({
      shopifyOrderId: Number(order_id),
      shopifyOrderNumber: order.order_number ? `#${order.order_number}` : `#${order_id}`,
      customerEmail: order.email || '',
      customerName,
      type: isDeuil ? 'deuil' : 'festivites',
      briefData: brief,
    ...
    console.log(`[Dorémi /formulaire] Brief sauvegardé en Supabase — ID: ${dbOrder.id}`);
  } catch (e) {
    console.error('[Dorémi /formulaire] Erreur Supabase (brief):', e.message);
    // On continue quand même — le flow Shopify reste fonctionnel
  }
  ...
  // ── Sauvegarder les paroles en Supabase ──
  try {
    await db.saveLyrics(Number(order_id), paroles);
    console.log(`[Dorémi /formulaire] Paroles sauvegardées en Supabase pour commande ${order_id}`);
  } catch (e) {
    console.error('[Dorémi /formulaire] Erreur Supabase (paroles):', e.message);
  }
